...
void afisare(VEHICUL *veh)
{
    printf("%s - %s - %s - %d\n",veh->judet,veh->marca,veh->denumire,veh->total);
}

uint8_t processLine(char *line,VEHICUL *data)
{
    char *field=NULL;
    field=strtok(line,",");
    if(field==NULL)
...
int32_t addAutoStatic(VEHICUL *array, VEHICUL *data, uint32_t size)
{
...
void printArray(VEHICUL *array,uint32_t size)
{
...
void printCars(VEHICUL *array,uint32_t size,const char *filename,char *marca)
{
    FILE *fout=NULL;
...
int main(int argc, char **argv)
{
    ...
    static VEHICUL array[300000]; // un tablou de tipuri de date AUTO declarat cu static pentru a nu depasi stiva
    VEHICUL e;
    uint32_t count = 0;
